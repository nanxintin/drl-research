...
flags.DEFINE_string('experiment', 'spaceInvaders', 'Name of the current experiment')
flags.DEFINE_string('game', 'SpaceInvaders-v0', 'Name of the atari game to play. Full list here: https://gym.openai.com/envs#atari')
flags.DEFINE_integer('num_concurrent', 8, 'Number of concurrent actor-learner threads to use during training.')
flags.DEFINE_integer('tmax', 10000000, 'Number of training timesteps.')
...
flags.DEFINE_string('summary_dir', './model/summaries', 'Directory for storing tensorboard summaries')
flags.DEFINE_string('checkpoint_dir', './model/checkpoints', 'Directory for storing model checkpoints')
...
flags.DEFINE_boolean('load_model', False, 'If true, load the previous model and weight to continue training')
...
flags.DEFINE_string('eval_dir', './model/', 'Directory to store gym evaluation')
...
def sample_final_epsilon():
    ...
    These values are specified in section 5.1 of the paper.
    """
...
def train(session, graph_ops, num_actions, saver):
    ...
    #restore model weights and continue training 
    if FLAGS.load_model:
        saver.restore(session, tf.train.latest_checkpoint('./model/checkpoints/'))
